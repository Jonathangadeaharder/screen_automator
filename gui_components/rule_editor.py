...
class RuleEditor(tb.Toplevel):
    ...
    def __init__(self, master: 'MainWindow', rule=None):
        ...
        self.cmb_monitor = tb.Combobox(frame, values=self._mon_options, state='readonly', textvariable=self.var_monitor)
        ...
        disable_after_spin = tb.Spinbox(disable_frame, from_=0, to=100, textvariable=self.disable_after_var, width=5)
        ...
        tb.Button(disable_frame, text=_('Browse...'), command=self._browse_disable_image).grid(row=1, column=2, padx=5)
        ToolTip(disable_image_entry, text=_('When this image is detected, the rule will be disabled'))
        ...
        tb.Button(action_toolbar, text=f"+ {_('Record')}", command=self._record_actions).pack(side=LEFT, padx=2)
        tb.Button(action_toolbar, text=f"⌨ {_('Key Press')}").pack(side=LEFT, padx=2)
        tb.Button(action_toolbar, text=f"📝 {_('Type Text')}", command=self._add_text).pack(side=LEFT, padx=2)
        
        tb.Button(action_toolbar, text=f"↩️ {_('Undo')}", command=self._undo).pack(side=RIGHT, padx=2)
        tb.Button(action_toolbar, text=f"↪️ {_('Redo')}", command=self._redo).pack(side=RIGHT, padx=2)
        tb.Button(action_toolbar, text=f"🗑 {_('Delete')}", command=self._delete_selected).pack(side=RIGHT, padx=2)
    ...
    def _save(self):
        ...
        if self.rule:
            ...
            new_rule = self.automator.rule_manager.create_rule(name=name, image_path='', actions=actions)
            self.automator.rule_manager.update_rule(
                new_rule.id, 
                monitor_idx=mon_idx,
                disable_after_executions=disable_after,
    ...
    def _browse_disable_image(self) -> None:
        ...
        filepath = filedialog.askopenfilename(
            title=_("Select Disable Trigger Image"),
            filetypes=filetypes
        )
