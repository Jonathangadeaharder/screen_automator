...
class ToggleSwitch(tk.Canvas):
    ...
    def __init__(self, parent, width=60, height=30, bg='#FFFFFF', fg='#0078D7', command=None, **kwargs):
        ...
        super().__init__(parent, width=width, height=height, background=parent_bg, highlightthickness=0, **kwargs)
...
from src.action_executor import (ActionExecutor, create_click_action, create_double_click_action,
    create_right_click_action, create_type_text_action, create_key_press_action,
    create_key_combination_action, create_wait_action, create_scroll_action, 
    create_click_image_action, Action, ActionType)
from src.screen_selector import ScreenSelector, KeyRecorder
from utils.logging import setup_gui_logging, get_logger
...
class ClickRecorder:
    ...
    def start_recording(self):
        ...
        messagebox.showinfo("Recording", "Click on screen positions to record. Press ESC to stop recording.")
...
class ScreenAutomatorGUI:
    ...
    def __init__(self):
        ...
        version_label = ttk.Label(status_bar, text="v1.0", font=self.small_font, foreground="#888888")
    ...
    def set_style(self):
        ...
        style.configure("Modern.TNotebook.Tab", background=bg_color, padding=[10, 2],
                      font=('Segoe UI', 10))
        ...
        style.configure("Treeview", background="white", foreground=text_color,
                      fieldbackground="white", borderwidth=1, rowheight=25)
    ...
    def setup_rules_tab(self):
        ...
        new_btn = ttk.Button(primary_buttons, text="+ New Rule", command=self.new_rule, style='Accent.TButton')
        new_btn.pack(side=tk.LEFT, padx=1)
        
        ttk.Button(primary_buttons, text="Edit", command=self.edit_rule).pack(side=tk.LEFT, padx=1)
        ttk.Button(primary_buttons, text="Delete", command=self.delete_rule).pack(side=tk.LEFT, padx=1)
        
        # Secondary action buttons
        secondary_buttons = ttk.Frame(toolbar)
        secondary_buttons.pack(side=tk.LEFT)
        
        ttk.Button(secondary_buttons, text="Test", command=self.test_rule).pack(side=tk.LEFT, padx=1)
        ttk.Button(secondary_buttons, text="🔥 Fire Up", command=self.fire_up_rule, style='Accent.TButton').pack(side=tk.LEFT, padx=1)
        ttk.Button(secondary_buttons, text="Refresh", command=self.refresh_rules).pack(side=tk.LEFT, padx=1)
        ...
        toggle_all_btn = ttk.Button(toggle_all_frame, text="🔄 Toggle All (Ctrl+T)", command=self.toggle_all_rules)
        ...
        self.rules_tree = ttk.Treeview(list_frame, columns=('Name', 'Status', 'Type', 'Trigger', 'Actions', 'Window'), show='tree headings')
        ...
        self.rules_tree.column('#0', width=70, minwidth=70, anchor='center')  # Toggle column (leftmost)
        ...
        self.rules_tree.column('Window', width=150, minwidth=100, anchor='w')  # Window Target column
    ...
    def setup_editor_tab(self):
        ...
        ttk.Entry(info_frame, textvariable=self.name_var, width=30).grid(row=0, column=1, padx=5, pady=2)
        
        ttk.Label(info_frame, text="Description:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=2)
        self.desc_var = tk.StringVar()
        ttk.Entry(info_frame, textvariable=self.desc_var, width=30).grid(row=1, column=1, padx=5, pady=2)
        ...
        ttk.Radiobutton(condition_frame, text="Trigger on Image Detection", variable=self.condition_var, value="image", command=self.on_condition_change).pack(anchor=tk.W, pady=2)
        ttk.Radiobutton(condition_frame, text="Trigger when Screen Unchanged", variable=self.condition_var, value="screen_unchanged", command=self.on_condition_change).pack(anchor=tk.W, pady=2)
        ...
        self.timeout_spinbox = ttk.Spinbox(timeout_frame, from_=0.5, to=60.0, increment=0.5, textvariable=self.timeout_var, width=10)
        ...
        select_btn = ttk.Button(select_frame, text="Select Target Window", 
                               command=self.select_target_window, 
                               style="Accent.TButton")
        select_btn.pack(side=tk.LEFT, padx=10)
        
        ttk.Label(select_frame, text="Click to choose which window this rule applies to").pack(side=tk.LEFT, padx=5)
        
        # Window info display
        info_frame = ttk.Frame(window_frame)
        info_frame.pack(fill=tk.X, pady=5)
        
        self.window_info_var = tk.StringVar(value="No window selected (rule will apply to any window)")
        window_info_label = ttk.Label(info_frame, textvariable=self.window_info_var, 
        ...
        cluster_combo['values'] = ('', 'browsers', 'text_editors', 'dev_tools', 'communication', 'media', 'utilities')
        ttk.Label(cluster_frame, text="(Optional - for organizing related rules)").pack(side=tk.LEFT, padx=5)
        ...
        ttk.Button(img_buttons, text="Select Image File", command=self.select_image_file).pack(side=tk.LEFT, padx=5)
        ttk.Button(img_buttons, text="Capture Screen Region", command=self.capture_screen_region).pack(side=tk.LEFT, padx=5)
        
        self.image_path_var = tk.StringVar()
        ttk.Label(self.image_frame, textvariable=self.image_path_var, foreground='blue').pack(pady=5)
        ...
        ttk.Button(action_row1, text="Record Clicks", command=self.record_clicks).pack(side=tk.LEFT, padx=2)
        ttk.Button(action_row1, text="Record Keys", command=self.record_keys).pack(side=tk.LEFT, padx=2)
        ttk.Button(action_row1, text="Add Wait", command=self.add_wait_action).pack(side=tk.LEFT, padx=2)
        
        # Row 2: Advanced actions and controls
        action_row2 = ttk.Frame(action_buttons)
        action_row2.pack(fill=tk.X)
        
        ttk.Button(action_row2, text="Add Manual", command=self.add_manual_action).pack(side=tk.LEFT, padx=2)
        ttk.Button(action_row2, text="Click on Image", command=self.add_click_image_action).pack(side=tk.LEFT, padx=2)
        ttk.Button(action_row2, text="Clear All", command=self.clear_actions).pack(side=tk.LEFT, padx=2)
        ...
        actions_scrollbar = ttk.Scrollbar(actions_list_frame, orient=tk.VERTICAL, command=self.actions_listbox.yview)
    ...
    def setup_monitor_tab(self):
        ...
        # Add sort button
        ttk.Button(header_controls, text="View Rules by Priority", 
                  command=self.show_priority_list).pack(side=tk.LEFT, padx=5)
        ...
        status_label = ttk.Label(status_frame, textvariable=self.status_var, font=('Segoe UI', 11, 'bold'))
        status_label.pack(side=tk.LEFT, padx=5)
        
        # Action counter indicator
        self.action_counter_var = tk.StringVar(value="")
        action_counter_label = ttk.Label(status_frame, textvariable=self.action_counter_var, font=self.normal_font, foreground="#666666")
        ...
        ttk.Label(settings_grid, text="Check Interval:", font=self.normal_font).grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
        ...
        ttk.Button(button_frame, text="Reset Counter", command=self.reset_action_counter).pack(side=tk.LEFT, padx=5)
    ...
    def log_message(self, message):
        ...
        if not hasattr(self, "_log_tags_created"):
            ...
            self.log_text.tag_configure("important_warning", foreground="#FF6600", font=('Segoe UI', 10, 'bold'))
    ...
    def refresh_rules(self):
        ...
        for rule in rules:
            ...
            if hasattr(rule, 'target_window_title') and rule.target_window_title:
                window_target = rule.target_window_title[:20] + ("..." if len(rule.target_window_title) > 20 else "")
            ...
            item_id = self.rules_tree.insert("", "end", text=toggle_text, 
    ...
    def edit_rule(self):
        ...
        if getattr(self.current_rule, 'target_window_class', ''):
            self.window_info_var.set(f"Target: {getattr(self.current_rule, 'target_window_class', '')}\n" +
                                  f"Using: Window Class")
        elif getattr(self.current_rule, 'target_window_process', ''):
            self.window_info_var.set(f"Target: {getattr(self.current_rule, 'target_window_process', '')}\n" +
    ...
    def fire_up_rule(self):
        ...
        self.log_message(f"🔥 Firing up rule '{rule_name}' - executing {len(rule.actions)} actions immediately...")
    ...
    def record_clicks(self):
        ...
        btn = ttk.Button(stop_window, text="Stop Recording", command=lambda: recording_active.set(False))
        ...
        def record_thread():
            try:
                ...
                clicks_recorded = recorder.record_until(lambda: not recording_active.get(), exclude_region=stop_button_region)
            except Exception as e:
    ...
    def add_wait_action(self):
        """Add wait action"""
        duration = simpledialog.askfloat("Wait Duration", "Enter wait duration in seconds:", initialvalue=1.0)
        if duration:
    ...
    def add_click_image_action(self):
        ...
        if messagebox.askyesno("Image Selection", 
    ...
    def action_to_string(self, action):
        ...
        if action.type == ActionType.CLICK:
            ...
            text = action.params['text'][:50] + "..." if len(action.params['text']) > 50 else action.params['text']
            return f"Type: '{text}'"
    ...
    def save_rule(self):
        ...
        if condition_type == "image":
            if not image_path or not os.path.exists(image_path):
                messagebox.showerror("Error", "Valid trigger image is required for image-based rules")
                return
        else:  # screen_unchanged
            if timeout <= 0:
                messagebox.showerror("Error", "Timeout must be greater than 0 for screen unchanged rules")
                return
    ...
    def on_window_method_change(self, event=None):
        ...
        if method == "class":
            ...
            self.log_message("💡 Class-based targeting is the most reliable method for window identification")
        elif method == "process":
            self.target_window_process_var.set(self.target_window_process_var.get())
            # Show a tip about process-based targeting
            self.log_message("💡 Process-based targeting works well for applications with changing window titles")
        else:  # auto
    ...
    def start_monitoring(self):
        ...
        if not self.automator.is_running():
            ...
            # Add debug logging
            automator_type = "Context-aware" if isinstance(self.automator, ContextAwareAutomator) else "Standard"
            ...
            if enabled_rules:
                self.log_message(f"📋 Found {len(enabled_rules)} enabled rules: {[r.name for r in enabled_rules[:3]]}{'...' if len(enabled_rules) > 3 else ''}")
                
                # Show context-aware information
                if isinstance(self.automator, ContextAwareAutomator):
                    self.log_message(f"🗂️  Context-aware monitoring enabled - rules will target specific windows")
            else:
                self.log_message("⚠️  No enabled rules found - monitoring will run but no automation will occur")
    ...
    def show_priority_list(self):
        ...
        ttk.Label(header_frame, text="Rules Sorted by Priority", 
                 font=self.heading_font).pack(side=tk.LEFT)
        
        ttk.Label(header_frame, 
                 text="(Lower number = higher priority)", 
                 font=self.small_font).pack(side=tk.RIGHT)
        ...
        for rule in rules:
            ...
            tree.insert("", tk.END, text=rule.name, values=(rule.priority, status, len(rule.actions)))
        
        # Add info text
        info_text = ("Rules are executed in priority order. \n"
        ...
        info_label = ttk.Label(main_frame, text=info_text, 
                              font=self.small_font, foreground="#555555", 
    ...
    def select_target_window(self):
        ...
        try:
            ...
            ttk.Label(frame, text="Choose which window this rule applies to:", 
                    font=self.heading_font).pack(pady=(0, 5))
            ttk.Label(frame, text="The rule will only run when this window is active").pack(pady=(0, 15))
            ...
            ttk.Checkbutton(any_window_frame, text="Apply to any window", 
                          variable=any_window_var).pack(side=tk.LEFT)
                ...
                if any_window_var.get():
                    ...
                    if selection:
                        ...
                        if hasattr(window, 'class_name') and window.class_name:
                            ...
                            self.window_info_var.set(f"Target: {window.title}\n" + 
                        ...
                        messagebox.showinfo("No Selection", "Please select a window or check 'Apply to any window'")
                        return
            ...
            ttk.Button(button_frame, text="Apply", command=on_select, 
...
class ActionDialog:
    ...
    def setup_dialog(self):
        ...
        for text, value in actions:
            ttk.Radiobutton(action_frame, text=text, variable=self.action_var, 
                           value=value, command=self.on_action_change).pack(anchor=tk.W)
    ...
    def on_action_change(self):
        ...
        if action_type in ["click", "double_click", "right_click"]:
            ttk.Label(self.params_frame, text="X:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
            self.x_var = tk.IntVar()
            ttk.Entry(self.params_frame, textvariable=self.x_var, width=10).grid(row=0, column=1, padx=5, pady=5)
            
            ttk.Label(self.params_frame, text="Y:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
            self.y_var = tk.IntVar()
            ttk.Entry(self.params_frame, textvariable=self.y_var, width=10).grid(row=1, column=1, padx=5, pady=5)
        
        elif action_type == "type_text":
            ttk.Label(self.params_frame, text="Text:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
            self.text_var = tk.StringVar()
            ttk.Entry(self.params_frame, textvariable=self.text_var, width=30).grid(row=0, column=1, padx=5, pady=5)
        
        elif action_type == "key_press":
            ttk.Label(self.params_frame, text="Key:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
            self.key_var = tk.StringVar()
            ttk.Entry(self.params_frame, textvariable=self.key_var, width=20).grid(row=0, column=1, padx=5, pady=5)
        
        elif action_type == "key_combo":
            ttk.Label(self.params_frame, text="Keys (comma-separated):").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
            self.keys_var = tk.StringVar()
            ttk.Entry(self.params_frame, textvariable=self.keys_var, width=30).grid(row=0, column=1, padx=5, pady=5)
        
        elif action_type == "wait":
            ttk.Label(self.params_frame, text="Duration (seconds):").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
            self.duration_var = tk.DoubleVar(value=1.0)
            ttk.Entry(self.params_frame, textvariable=self.duration_var, width=10).grid(row=0, column=1, padx=5, pady=5)
        
        elif action_type == "scroll":
            ttk.Label(self.params_frame, text="Clicks:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
            self.clicks_var = tk.IntVar(value=3)
            ttk.Entry(self.params_frame, textvariable=self.clicks_var, width=10).grid(row=0, column=1, padx=5, pady=5)
...
class KeyInputDialog:
    ...
    def setup_dialog(self):
        ...
        ttk.Radiobutton(type_frame, text="Type Text", variable=self.action_type_var, value="text", command=self.on_action_change).pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(type_frame, text="Key Combination", variable=self.action_type_var, value="combo", command=self.on_action_change).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Button(button_frame, text="OK", command=self.ok_clicked, style="Accent.TButton").pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Cancel", command=self.cancel_clicked).pack(side=tk.LEFT, padx=5)
    ...
    def ok_clicked(self):
        ...
        if not input_value:
            messagebox.showwarning("Input Required", "Please provide an input value.", parent=self.dialog)
            return
