...
from gui_components.dialogs import TipDialog, FirstRuleWizard, PerformanceOverlay
from gui_components.widgets import ColorBox, ConfirmDialog, SearchEntry, CollapsiblePane, StatusIndicator, KeybindField
...
class MainWindow(tb.Window):
    ...
    def _build_toolbar(self):
        ...
        tb.Button(toolbar, text=f'➕ {_("New")}', bootstyle=PRIMARY, command=self._new_rule).pack(side=LEFT, padx=2)
        tb.Button(toolbar, text=f'✏ {_("Edit")}', command=self._edit_rule).pack(side=LEFT, padx=2)
        tb.Button(toolbar, text=f'🗑 {_("Delete")}', bootstyle=DANGER, command=self._delete_rule).pack(side=LEFT, padx=2)
        tb.Button(toolbar, text=f'▶ {_("Test")}', bootstyle=SUCCESS, command=self._test_rule).pack(side=LEFT, padx=2)
        self.btn_conflicts = tb.Button(toolbar, text='⚠ 0', bootstyle=WARNING, command=self._show_conflicts_dialog)
        ...
        self.btn_toggle_monitor = tb.Button(toolbar, text=_('Show Monitor'), command=self._toggle_monitor)
