...
@cli.command()
@click.option(
    "--timeout", type=int, default=0, help="Stop monitoring after N seconds (0 = no timeout)"
)
@click.pass_context
...
@click.option(
    "--condition",
    type=click.Choice(["image", "screen_unchanged"]),
    default="image",
    help="Rule condition type",
)
@click.option(
    "--timeout", type=float, default=5.0, help="Timeout in minutes for screen_unchanged condition"
)
@click.pass_context
def create_rule(ctx, name, image, description, condition, timeout):
    ...
    if condition == "image":
        ...
        click.echo(
            f"Creating rule '{name}' that triggers when screen is unchanged for {timeout} minutes"
        )
    ...
    if condition == "image":
        click.echo(
            f"✅ Image-based rule '{rule.name}' created successfully! (ID: {rule.id[:8]}...)"
        )
    else:
        click.echo(
            f"✅ Screen unchanged rule '{rule.name}' created successfully! (ID: {rule.id[:8]}...)"
        )
...
def test_rule_window(ctx, rule_id, title, process):
    ...
    if not title and not process:
        ...
        if not title and not process:
            click.echo(
                "❌ Please specify --title or --process, or set window targeting on the rule"
            )
            return
    ...
    try:
        result = automator.test_rule_in_window(
            rule.id, window_title=title or "", window_process=process or ""
        )
        if result:
