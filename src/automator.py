...
class ScreenAutomator:
    def __init__(self, rules_dir: str = "data/rules"):
        ...
        self.screen_unchanged_rules: Dict[str, float] = {}  # Maps rule_id to start time when screen first became unchanged
    ...
    def _monitor_loop(self):
        ...
        while self.running:
            try:
                ...
                screen_unchanged_rules = [r for r in enabled_rules if r.condition_type == "screen_unchanged"]
                ...
                if image_rules:
                    ...
                    if -1 in rules_by_monitor:
                        ...
                        self._process_rules_on_image(rules_by_monitor[-1], full_img, 0, 0, current_time)
        ...
        try:
            ...
            for rule in rules:
                ...
                if time_unchanged >= timeout_seconds:
                    print(f"Screen unchanged for {rule.screen_unchanged_timeout:.1f} minutes, executing rule '{rule.name}'")
    ...
    def _process_rules_on_image(self, rules: List[Rule], img, offset_x: int, offset_y: int, current_time: float):
        ...
        for rule in rules:
            ...
            try:
                ...
                if match:
                    ...
                    x += offset_x
                    y += offset_y
                    print(f"Found match for rule '{rule.name}' at {(x, y, width, height)} (monitor offset {offset_x},{offset_y})")
                    ...
                    if rule.capture_width > 0 and rule.capture_height > 0:
                        ...
                        distance = ((match_center_x - capture_center_x) ** 2 + 
                                   (match_center_y - capture_center_y) ** 2) ** 0.5
                        ...
                        if distance > max_distance:
                            print(f"Sanity check failed: Match at {(x, y, width, height)} is too far from capture area "
                    ...
                    if self._is_mouse_idle() and is_valid_match:
                        ...
                        if time_since_last_exec < self.min_rule_interval:
                            print(f"Skipping rule '{rule.name}' - executed too recently ({time_since_last_exec:.1f}s ago)")
                            continue
