...
class WindowManager:
                ...
                for line in result.stdout.strip().split('\n'):
                    if line:
                        ...
                        if len(parts) >= 8:
                            ...
                            process_id = 0
                            try:
                                pid_result = subprocess.run(['xprop', '-id', hex(window_id), '_NET_WM_PID'], 
                                                          capture_output=True, text=True, timeout=2)
                                if pid_result.returncode == 0 and '=' in pid_result.stdout:
                                    process_id = int(pid_result.stdout.split('=')[1].strip())
                                    process = psutil.Process(process_id)
                                    process_name = process.name()
                            except (subprocess.TimeoutExpired, psutil.NoSuchProcess, psutil.AccessDenied, ValueError):
                                pass
    ...
    def get_window_screenshot(self, window_info: WindowInfo) -> Optional[Any]:
        ...
        if self.system == "Windows" and HAS_WIN32:
            ...
            print(f"Windows-specific capture failed for '{window_info.title}', trying fallback method...")
            # Fallback to region-based screenshot
            screenshot = self._get_window_screenshot_fallback(window_info)
            if screenshot:
                return screenshot
            
            print(f"Fallback capture also failed for '{window_info.title}', trying screen capture...")
