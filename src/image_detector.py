...
class ImageDetector:
    ...
    def find_image_on_screen(self, template_path: str, screen_image: Optional[np.ndarray] = None) -> Optional[Tuple[int, int, int, int]]:
        """
        ...
        if template.shape[0] < 10 or template.shape[1] < 10:
            print(f"Warning: Template image is very small ({template.shape[1]}x{template.shape[0]})")
    ...
    def find_all_matches(self, template_path: str, screen_image: Optional[np.ndarray] = None) -> List[Tuple[int, int, int, int]]:
    ...
    def wait_for_image(self, template_path: str, timeout: float = 10.0, check_interval: float = 0.5) -> Optional[Tuple[int, int, int, int]]:
