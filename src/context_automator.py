...
class ContextAwareAutomator(ScreenAutomator):
    ...
    def __init__(self, rules_dir: str = "data/rules"):
        ...
        self.on_window_context_changed: Optional[Callable[[Optional[WindowInfo], Optional[WindowInfo]], None]] = None
        ...
        if self.current_context_window:
            ...
            if self.window_manager.restore_original_window():
                ...
                if HAS_PYAUTOGUI and self.original_cursor_pos:
                    try:
                        pyautogui.moveTo(self.original_cursor_pos[0], self.original_cursor_pos[1], duration=0.2)
    ...
    def _execute_rule_with_context(self, rule: Rule, current_time: float) -> bool:
        ...
        try:
            ...
            needs_context_switch = (
                hasattr(rule, 'target_window_class') and rule.target_window_class or
                hasattr(rule, 'target_window_process') and rule.target_window_process or  
                hasattr(rule, 'target_window_title') and rule.target_window_title
            )
    ...
    def _get_rule_target_window(self, rule: Rule) -> Optional[WindowInfo]:
        ...
        try:
            ...
            if window_id_method == 'class' and hasattr(rule, 'target_window_class') and rule.target_window_class:
                window = self.window_manager.find_window_by_class(rule.target_window_class)
                if window:
                    return window
            
            # Process-based targeting
            elif window_id_method == 'process' and hasattr(rule, 'target_window_process') and rule.target_window_process:
    ...
    def _switch_to_context_window(self, target_window: WindowInfo) -> bool:
        ...
        for i, rule in enumerate(cluster_rules):
            ...
            if window_id_method == 'class' and getattr(rule, 'target_window_class', ''):
                ...
                windows = self.window_manager.find_windows_by_process(rule.target_window_process)
                if windows:
                    print(f"✅ Found {len(windows)} windows with process '{rule.target_window_process}'")
            ...
            # Auto method or fallback: try all available targeting methods
                ...
                if rule.target_window_process:
                    print(f"🔍 Auto: Searching for windows with process: '{rule.target_window_process}'")
                    windows = self.window_manager.find_windows_by_process(rule.target_window_process)
                    if windows:
                        print(f"✅ Found {len(windows)} windows with process '{rule.target_window_process}'")
                        print(f"   Using first window: {windows[0].title}")
                ...
                if getattr(rule, 'target_window_title', ''):
                    print(f"🔍 Auto: Searching for window with title (legacy): '{rule.target_window_title}'")
                    window = self.window_manager.find_window_by_title(
                        rule.target_window_title, 
                        exact_match=getattr(rule, 'window_exact_match', False)
                    )
                    if window:
                        ...
                        print(f"ℹ️ Note: Title-based targeting is deprecated. Consider using class or process targeting.")
                        return window
    ...
    def _switch_to_context_window(self, target_window: WindowInfo) -> bool:
        ...
        try:
            ...
            switching_context = (not self.current_context_window or 
                                 self.current_context_window.handle != target_window.handle)
    ...
    def _execute_rule_in_context(self, rule: Rule, current_time: float) -> bool:
        ...
        try:
            ...
            if rule.condition_type == "screen_unchanged":
                print(f"⏱️ Checking screen unchanged condition (timeout: {rule.screen_unchanged_timeout} min)")
        ...
        try:
            ...
            if self.current_context_window:
                ...
                if screenshot is None:
                    print(f"⚠️ Window capture failed for image rule '{rule.name}', using full screen capture")
            ...
            for enabled_rule in self.rule_manager.list_enabled_rules():
                if enabled_rule.disable_on_image and os.path.exists(enabled_rule.disable_on_image):
                    disable_result = self.image_detector.find_image_on_screen(enabled_rule.disable_on_image, screenshot)
                    if disable_result:
                        print(f"⚠️ Disable image found for rule '{enabled_rule.name}', disabling rule")
    ...
    def _check_screen_unchanged_rule_in_context(self, rule: Rule, current_time: float) -> bool:
        ...
        try:
            ...
            if self.current_context_window:
                screenshot = self.window_manager.get_window_screenshot(self.current_context_window)
                if screenshot is None:
                    print(f"⚠️ Could not capture screenshot of window '{self.current_context_window.title}' for rule '{rule.name}'")
                    ...
                    window_name = f"{self.current_context_window.title} (using full screen fallback)"
                else:
            ...
            if hash_key not in self._context_hashes or self._context_hashes[hash_key] != window_hash:
                self._context_hashes[hash_key] = window_hash
                self._context_unchanged_start[hash_key] = current_time
                print(f"📸 Window content changed in '{window_name}' for rule '{rule.name}' - resetting timer")
                return False
            ...
            if unchanged_duration >= timeout_seconds:
                print(f"✅ Rule '{rule.name}' triggered - '{window_name}' content unchanged for {unchanged_duration:.1f}s (threshold: {timeout_seconds:.1f}s)")
                ...
                print(f"📸 Rule '{rule.name}' - '{window_name}' content unchanged for {unchanged_duration:.1f}s, need {remaining_time:.1f}s more")
    ...
    def _execute_rule_actions_in_context(self, rule: Rule, trigger_x: int = 0, trigger_y: int = 0) -> bool:
    ...
    def test_rule_in_window(self, rule_id: str, window_title: str = "", window_process: str = "") -> bool:
    ...
    def get_cursor_info(self) -> Dict[str, any]:
        ...
        info = {
            ...
            "current_context_window": self.current_context_window.title if self.current_context_window else None
        }
    ...
    def get_debug_info(self) -> Dict[str, any]:
        ...
        try:
            ...
            info["windows"] = {
                ...
                "active_window": self.window_manager.get_active_window().title if self.window_manager.get_active_window() else None
            }
        except Exception as e:
