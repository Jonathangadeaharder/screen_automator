...
def test_window_screenshot_methods():
    ...
    image_paths = [
        "test_windows_screenshot.png",
        "test_pyautogui_region.png",
...
def test_image_detection():
    ...
    if not os.path.exists(test_image_path):
        ...
        print(
            f"Creating test target image from screen region: {region_x},{region_y} {region_width}x{region_height}"
        )
...
def main():
    ...
    print(
        "1. For window screenshots: Use the Windows direct method if it works, otherwise use PyAutoGUI region capture."
    )
    print(
        "2. For image detection: Use the method that successfully found the test image with highest confidence."
    )
