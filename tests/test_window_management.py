...
class TestWindowManager:
    ...
    def test_window_info_to_dict(self, mock_window_info):
        ...
        expected_keys = {
            'handle', 'title', 'process_name', 'process_id',
            'x', 'y', 'width', 'height', 'is_visible', 'is_minimized', 'class_name'
        }
...
class TestRuleManagerWindowExtensions:
    ...
    def test_get_rules_by_cluster(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            rule3 = rm.create_rule(
                name="Calculator Rule",
                image_path="calc.png", 
                actions=actions
            )
    ...
    def test_rule_matches_window(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert rm._rule_matches_window(
                MagicMock(target_window_title="Notepad", target_window_process="", window_exact_match=False),
            ...
            assert rm._rule_matches_window(
                MagicMock(target_window_title="Calculator", target_window_process="", window_exact_match=True),
                "Calculator",
                "calc.exe"
            )
            ...
            # Test process match
            assert rm._rule_matches_window(
                MagicMock(target_window_title="", target_window_process="chrome", window_exact_match=False),
                "Google Chrome",
                "chrome.exe"
            )
            ...
            assert rm._rule_matches_window(
                MagicMock(target_window_title="", target_window_process="", window_exact_match=False),
                "Any Window",
                "any.exe"
            )
...
class TestContextAwareAutomator:
    ...
    def test_get_cluster_target_window(self, mock_windows_list):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            automator.window_manager.find_window_by_title = MagicMock(return_value=mock_windows_list[0])
            automator.window_manager.find_windows_by_process = MagicMock(return_value=[mock_windows_list[1]])
    ...
    def test_get_available_windows(self, mock_windows_list):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(automator.window_manager, 'get_running_windows', return_value=mock_windows_list):
...
class TestWindowManagementIntegration:
    ...
    def test_rule_window_targeting_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            rule = automator.rule_manager.create_rule(
                name="Notepad Click Rule",
                image_path="notepad_button.png",
                actions=actions
            )
